...
def train(self):
    ...
    for i, (x_batch, datum) in enumerate(batches):
        ...
        if not ckpt:
            # Evaluate trainset
            predict(self, self.FLAGS.dataset)
            evaluate(self, self.FLAGS.annotation, os.path.join(self.FLAGS.imgdir, 'out'),
                     "results_train" + str(step_now) + ".txt")
            '''
            Maybe we have to delete 'out' folder for next evaluation of the testset. Please check!
            For further convinience, we should change the postprocess function on predict.py 
            because the default output is: os.path.join(self.FLAGS.imgdir, 'out')
            Contact me on telegram for the details -_-
            '''
            # Evaluate testset
            predict(self, self.FLAGS.imgdir)
            mAP = evaluate(self, self.FLAGS.imgdir, os.path.join(self.FLAGS.imgdir, 'out'),
                           "results_test" + str(step_now) + ".txt")
            if mAP > best_mAP:
                best_mAP = mAP
